...
# Output folder updated to reflect raw amplitude matching
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "output_reference_new_masses_cross_sections")
...
INPUT_MODELS = [
        (2, 0.001, 1.4000e-26, None, 7.563e-25),
        (2, 0.1, 2.42e-24, None, 1.664e-22),
        (4, 0.001, 1.82414e-21, None, 2.37415e-24),
        (4, 0.1, 3.27388e-20, None, 1.9032e-17)
]
...
def create_ini_production(template_path, params_to_override, output_path, k_res):
    ...
    # Using the dynamically passed k_res here:
...
def main():
    ...
    with open(PROGRESS_LOG_FILE, 'a') as prog_log:
        for n, mass, sig_hm, sig_59, sig_env in INPUT_MODELS:
            ...
            # I discovered that n=2 has a lower resolution at 80 k_per_decade and n=4 at 150 k_per_decade
