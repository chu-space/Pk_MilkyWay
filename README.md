...
You do this by finding the envelope cross-section, at which the transfer function or the ratio between the matter power spectrum and the benchmark cold dark matter (CDM) power spectrum. To make sure that the matter power spectrum is suppressed correctly, I have matched the cosmological parameters with the transfer function of the IDM spectra first - each normalised by the CDM power spectra.

With this directory, you can gain access information to transfer functions which tell the story of matter suppression of different models for both the intermediate mass (researching in between the previous suspected masses of IDM) or interpolated (a cross-section evaluated at the latest 5.9kev WDM observational constraint line) cross-sections match the expected transfer function behaviour we expect. Both of these new searches provides new insight on the momentum and cross-section dependence a range of particle model of dark matter would have. 
